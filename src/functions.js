...
const safeNeighborhood = (board, row, column) => {
    ...
    // acumula o resultado de sucessivas operacoes 'safes' em cada elemento do array de vizinhos
...
// transforma a matriz em array linear de fields
const fields = board => [].concat(...board)
const hasExplosion = board => fields(board).filter(field => field.exploded).length > 0
const pendding = field => (field.mined && !field.flagged) || (!field.mined && !field.openned)
const wonGame = board => fields(board).filter(pendding).length === 0
const showMines = board => fields(board).filter(field => field.mined).forEach(field => field.openned = true)
